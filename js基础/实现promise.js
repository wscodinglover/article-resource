var PromisePolyfill = (function () {
  ...
  function tryToResolve(value) {
    ...
    // 保证S6之前的 polyfill 也能和ES6的原⽣promise混⽤
    if (
      value !== null &&
      (typeof value === "object" || typeof value === "function")
    ) {
      try {
        // 这⾥记录这次then的值同时要被try包裹
        // 主要原因是 then 可能是⼀个getter, 也也就是说
        // 1. value.then可能报错
        // 2. value.then可能产⽣副作⽤(例如多次执⾏可能结果不同)
        var then = value.then;
        // 另⼀⽅⾯, 由于⽆法保证 then 确实会像预期的那样只调⽤⼀个onFullfilled / onRejected
        // 所以增加了⼀个flag来防⽌resolveOrReject被多次调⽤
        ...
        if (typeof then === "function") {
          ...
          // 拥有then 但是then不是⼀个函数 所以也不是thenable
