...
function ajax(t, e, s = null) {
  const i = new XMLHttpRequest();
  return new Promise((n, o) => {
    (i.onreadystatechange = function () {
      4 === i.readyState &&
        (200 === i.status ? n(i.responseText) : o(i.status));
    }),
      i.open(t, e),
      i.send(s);
  });
...
function info(id, txt, times) {
  $(id).innerHTML = txt;
  shake($(id), "shake-red", times);
  setTimeout(function () {
    $(id).innerHTML = "";
  }, times * 1000 + 2000);
}
...
function success() {
  ...
  const t2 = { end_time: end_time, end_level: localStorage.getItem("level") };
  ajax("POST", "/api/end/", JSON.stringify(t2)).then((t) => {
    upload();
  });
}
var _level;
...
function oneclick(bg, bh) {
  ...
  ajax("POST", "/api/start/", JSON.stringify(t1)).then((t) => {});
  var E;
  var j, h;
  var bf = RB;
  for (E = 8; E >= 0 && bf > 0; E--) {
...
$("uid").onclick = function () {
  if (_input == 0) {
    ...
    $("uid").innerHTML =
      '<input type="text" id="name" onchange="savename()" value=' + uid + ">";
  }
  _input = 1;
};
function savename() {
  shake($("uid"), "shake-red", 3);
  var name = $("name").value;
