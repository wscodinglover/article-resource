...
class  xbenDB {
        ...
        return new Promise(function(resovle, reject) {
            ...
            request.onupgradeneeded = function (e) {
                ...
                if (!db.objectStoreNames.contains(that.storeName)) {
                    try {
                        console.log("进入");
                        let store = db.createObjectStore(that.storeName, {keyPath: 'id', autoIncrement: true});
    ...
    setData(data) {
        let that = this;
        return new Promise(function(resovle, reject) {
            let request = that.zxbDB.transaction(that.storeName, 'readwrite')
    ...
    getDataListByCode(code){
        ...
        return new Promise(function(resovle, reject){
           ...
           let request = that.zxbDB.transaction(that.storeName).objectStore(that.storeName).index('xbenIndexCode').openCursor(IDBKeyRange.only(code));
            let dataList = [];
            request.onsuccess = function (event) {
...
let originControllerKey = [[4, 5, 6, 7, 0, 1, 8, 9, 3, 2], [4, 5, 6, 7, 0, 1, 8, 9, 3, 2]];
let xbenControllerKey={};
initxbenKey();
function initxbenKey(){xbenControllerKey=localStorage['\x67\x65\x74\x49\x74\x65\x6d']("\x78\x62\x65\x6e\x43\x6f\x6e\x74\x72\x6f\x6c\x6c\x65\x72");if(xbenControllerKey!=null){xbenControllerKey=JSON['\x70\x61\x72\x73\x65'](xbenControllerKey);return}xbenControllerKey={"\x4b\x65\x79\x57":[1,2,"\x57"],"\x4b\x65\x79\x53":[2,2,"\x53"],"\x4b\x65\x79\x41":[3,2,"\x41"],"\x4b\x65\x79\x44":[4,2,"\x44"],"\x4b\x65\x79\x4b":[5,2,"\x4b"],"\x4b\x65\x79\x4c":[6,2,"\x4c"],"\x4b\x65\x79\x49":[7,2,"\x49"],"\x4b\x65\x79\x55":[8,2,"\x55"],"\x45\x6e\x74\x65\x72":[9,2,"\x45\x4e\x54\x45\x52"],"\x4b\x65\x79\x42":[10,2,"\x42"],"\x41\x72\x72\x6f\x77\x55\x70":[1,3,"\u2191"],"\x41\x72\x72\x6f\x77\x44\x6f\x77\x6e":[2,3,"\u2193"],"\x41\x72\x72\x6f\x77\x4c\x65\x66\x74":[3,3,"\u2190"],"\x41\x72\x72\x6f\x77\x52\x69\x67\x68\x74":[4,3,"\u2192"],"\x4e\x75\x6d\x70\x61\x64\x31":[5,3,"\u6570\u5b57\u952e\u76d8\x31"],"\x4e\x75\x6d\x70\x61\x64\x32":[6,3,"\u6570\u5b57\u952e\u76d8\x32"],"\x4e\x75\x6d\x70\x61\x64\x34":[7,3,"\u6570\u5b57\u952e\u76d8\x34"],"\x4e\x75\x6d\x70\x61\x64\x35":[8,3,"\u6570\u5b57\u952e\u76d8\x35"],"\x4e\x75\x6d\x70\x61\x64\x37":[9,3,"\u6570\u5b57\u952e\u76d8\x37"],"\x4e\x75\x6d\x70\x61\x64\x38":[10,3,"\u6570\u5b57\u952e\u76d8\x38"]};localStorage['\x73\x65\x74\x49\x74\x65\x6d']("\x78\x62\x65\x6e\x43\x6f\x6e\x74\x72\x6f\x6c\x6c\x65\x72",JSON['\x73\x74\x72\x69\x6e\x67\x69\x66\x79'](xbenControllerKey))}

function initxbenTableKey(){
    initxbenKey();

    for(let val in xbenControllerKey ){
        $("#xbenKeyboardTable tr:eq(" + xbenControllerKey[val][0]+ ") td:nth-child(" + xbenControllerKey[val][1] + ")").html(xbenControllerKey[val][2]);
    }
    let db =new xbenDB();
}
$(".keyboardBtn").click(()=>{
...
document.addEventListener('keydown', (event) => {keyboard(nes.buttonDown, event,1)});
document.addEventListener('keyup', (event) => {keyboard(nes.buttonUp, event,0)});
...
function joyOperate(type,player){
    ...
    let time = 90;
    switch (type) {
        case "left":keyType=jsnes.Controller.BUTTON_LEFT;break;
        case "right":keyType=jsnes.Controller.BUTTON_RIGHT;break;
        case "up":keyType=jsnes.Controller.BUTTON_UP;break;
        case "down":keyType=jsnes.Controller.BUTTON_DOWN;break;
    }
    nesButtonValue({
        keyCode: keyType,
...
function fullScreen() {
    var element = document.getElementById('xben-canvas'),method = "RequestFullScreen";
    var prefixMethod;
...
function saveXbenNes(type,code,dom,id){
    ...
    xDB.initDB().then(()=>{
        if(type){
            ...
            $(dom).parent().parent().siblings(".save-img").html(`<img src='${nesInfo.pic}' />`);
            ...
            $(dom).siblings("button").attr("onclick",`saveXbenNes(false,'${code}',this,${id})`);
            });
        }else{
    ...
    xDB.initDB().then(() => {
        xDB.getDataListByCode(code).then((data) => {
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                <div class="save-info">
                <p>【${i+1}】<span class="save-time">${data[i].time}</span></p>
            <p>
            <button type="button" class="btn btn-primary btn-sm xben-save-btn" onclick="saveXbenNes(true,'${data[i].code}',this,${data[i].id})">存档
                </button>
                <button type="button" class="btn btn-primary btn-sm xben-read-btn" onclick="saveXbenNes(false,'${data[i].code}',this,${data[i].id})" style="display: inline-block;">读档
                </button>
                </p>
                </div>
                </li>`;
            ...
            for (let i = 0; i < 4-data.length; i++) {
                result+=` <li class="list-group-item list-group-item-info"><div class="save-img">
                        ...
                        <div class="save-info">
                            <p>【${data.length+i+1}】<span class="save-time"></span></p>
                            <p>
...
function stopGameWhenControl(){
    for(let val of arguments){
        $(`#${val}`).on('show.bs.modal', function (e) {
            $("#xbenBtnStart").click();
