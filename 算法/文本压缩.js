...
(function (win) {
  ...
  SF.prototype._heatChineseWords = function () {
    ...
    wordsData.wordsLv3 = this._mergeEtyma(
      wordsData.wordsLv2,
      wordsLv2,
      content
    );
    wordsData.wordsLv4 = this._mergeEtyma(
      wordsData.wordsLv3,
      wordsLv2,
      content
    );
    wordsData.wordsLv5 = this._mergeEtyma(
      wordsData.wordsLv4,
      wordsLv2,
      content
    );
    wordsData.wordsLv6 = this._mergeEtyma(
      wordsData.wordsLv5,
      wordsLv2,
      content
    );
    wordsData.wordsLv7 = this._mergeEtyma(
      wordsData.wordsLv6,
      wordsLv2,
      content
    );
    wordsData.wordsLv8 = this._mergeEtyma(
      wordsData.wordsLv7,
      wordsLv2,
      content
    );
    ...
    //序号范围约束
    var wordsRange = parseInt(1713 * 0.6);
  ...
  SF.prototype._extChineseChar = function (list) {
    ...
    //大于50的列队按一定量平均抽取汉字段组成新列队
      ...
      //需要抽取的长度
      var newLong = parseInt(Math.pow(list.length - 50, 0.82) + 50);
  ...
  SF.prototype._restoreResult = function (num, type) {
    ...
    if (type == "single") {
      ...
      this._data.result = this._data.result.replace(
        new RegExp(code, "g"),
        function () {
  ...
  SF.prototype.encode = function (source) {
    ...
    if (this._preTransition()) {
      ...
      throw new Error(
        "StrFolding.js: 太多的非英文字母！本压缩工具仅针对中英文文本使用。"
      );
    }
    //拼装结果
    var result =
      this._data.result +
      "|||" +
      this._data.listEN.join(",") +
      "|||" +
      this._data.listCN.join(",");
    //统计效率
    this._onEnd &&
      this._onEnd(this._statistics(startTime, source, result, "compress"));
    //立即清理
  ...
  SF.prototype.decode = function (result) {
    ...
    this._onEnd &&
      this._onEnd(
        this._statistics(startTime, result, this._data.result, "deCompress")
      );
    //立即清理
